...
export class AuthService implements IAuthService {
  ...
  async reqSignup(data: SignUpDataType): Promise<{tempToken: string}>
  {
    ...
    const tempToken = await this._tokenService.generateTempToken({email: data.email});
    await this._redisRepo.set<SignUpDataType>(`${RedisKeys.RESET_TOKEN}:${tempToken}`,data,180);
    await this._emailService.sendSignupOtp(data.email,otp);
    return { tempToken }
  }
  async verifyAndSignup(email: string, otp: string): Promise<{accessToken: string, refreshToken: string}>
  {
    const verify = await this._otpService.verifyOtp(`${RedisKeys.SIGNUP}:${email}`,otp);
    if(!verify)
    {
      throw new AppError(Messages.OTP_INVALID, HttpStatus.BAD_REQUEST);
    }
    const hashPassword = await this._passwordService.hashPassword(verify.password)
    const createdUser = await this._userService.createUser({name: verify.name, email: verify.email, passwordHash: hashPassword});
    const tokens = await this._tokenService.generateTokens({userId: createdUser.id});
  ...
  {
     const timeExceed = await this._redisRepo.get<SignUpDataType>(`${RedisKeys.RESET_TOKEN}:${tempToken}`);
     if(!timeExceed)
     {
      throw new AppError(Messages.TIME_EXCEEDED, HttpStatus.BAD_REQUEST);
     }
      const otp = await this._otpService.generateOtp();
    await this._otpService.storeOtp(`${RedisKeys.SIGNUP}:${timeExceed.email}`,{...timeExceed,otp},60);
    await this._emailService.sendSignupOtp(email,otp);

  }
  async login(data: LoginDataType): Promise<{accessToken: string, refreshToken: string}>
  {
    ...
    const check = await this._passwordService.comparePassword(userData.passwordHash,data.password);
    if(!check)
    {
      throw new AppError(Messages.INVALID_CREDENTIALS,HttpStatus.UNAUTHORIZED);
    }

    const tokens = await this._tokenService.generateTokens({userId: userData.id});
